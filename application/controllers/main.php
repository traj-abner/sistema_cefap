...
class Main extends CI_Controller {
	...
	public function index(){
            ...
            if ( $this->session->userdata('logged_in')) {
				...
				$msg->where('to_id',$this->session->userdata('id'))->order_by('data_envio','desc')->limit(2)->get();
				$data['received_messages'] = $msg;
				endif;
				
				$agn = new Agendamento();
				if ($data['uRole'] == CREDENCIAL_USUARIO_COMUM):
					$agn->include_related('facilities')->include_related('projetos')->where('usuario_id',$this->session->userdata('id'))->where('status <',AGENDAMENTO_STATUS_EXCLUIDO)->order_by('modified, created')->limit(3)->get();
				endif;
				if ($data['uRole'] == CREDENCIAL_USUARIO_ADMIN):
					$fcl = new Facility();
					$fcl->include_related('usuario')->where('usuario_id',$this->session->userdata('id'))->get();
					$agn->include_related('facilities')->include_related('projetos')->include_related('usuarios')->where_related_facility($fcl)->where('status <',AGENDAMENTO_STATUS_EXCLUIDO)->order_by('modified, created')->limit(3)->get();
				endif;
				if ($data['uRole'] == CREDENCIAL_USUARIO_SUPERADMIN):
					$fcl = new Facility();
					$fcl->include_related('usuario')->where('usuario_id',$this->session->userdata('id'))->get();
					$agn->include_related('facilities')->include_related('projetos')->include_related('usuarios')->where_related_facility($fcl)->order_by('modified, created')->limit(3)->get();
				endif;
				
				$bols = new Boleto();
				$bols->where('usuario_id',$this->session->userdata('id'))->order_by('data_vencimento','desc')->limit(3)->get();
				$data['bols'] = $bols;
